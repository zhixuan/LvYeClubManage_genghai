...
+ (NSString *)textWithFAIcon:(FMIconFont)fa_icon size:(CGFloat)size color:(UIColor *)color{
    
    UILabel *label = [[UILabel alloc] init];
    [FontAwesome label:label
               setIcon:[NSString fontAwesomeIconStringForEnum:fa_icon]
                  size:size
                 color:color sizeToFit:YES];
...
{
    label.font = [UIFont fontAwesomeFontOfSize:size];
    label.text = fa_icon;
...
//    NSAssert(fa_icon, @"You must specify an icon from font-awesome-codes.h.");
    ...
    return [self imageWithText:[NSString fontAwesomeIconStringForEnum:fa_icon]
                          font:[UIFont fontAwesomeFontOfSize:iconSize]
                     iconColor:iconColor
                     imageSize:imageSize];
}
                ...
                imageSize:(CGSize)imageSize;
{
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 6) {
        ...
        CGFloat fscale = 1.0;
//        if (HUIIsRetinaScreen()) {
//            fscale = 2.0;
//        }
        fscale = 2.0;
        UIGraphicsBeginImageContextWithOptions(imageSize, NO, fscale);
        {
