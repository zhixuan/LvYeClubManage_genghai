...
#import "LvyeClubRefundController.h"
#import "LvYeHTTPClient+ClubManage.h"
@interface LvyeClubRefundController ()

@end


@implementation LvyeClubRefundController
...
- (instancetype)init{
    ...
    if (self) {
        ...
        self.clubModuleName = [[NSString alloc]initWithFormat:@"%@",@"退款管理"];
        self.clubModuleImage = FMIconFontF0FA;
        self.clubModuleId = [[NSString alloc]initWithFormat:@"%@", @"9"];
    }
    return self;
}
    ...
    [KShareHTTPLvyeHTTPClient userClubOperationFunctionWithClubId:KLvyeProductClubSettings.clubLoginUserAtClubId userId:KLvyeProductClubSettings.clubloginUserPerId moduleId:self.clubModuleId completion:^(WebAPIResponse *response) {
        dispatch_async(dispatch_get_main_queue(), ^(void){
            ...
            if (response.code == WebAPIResponseCodeSuccess) {
                NSLog(@"response.code is %ld",(long)response.code);
